...
def process_image(img):
   ...
   # 3. Fit Lines (fitting x as a function of y: x = my + b)
    ...
    if len(left_x) > 400:
        ...
        if -5 < temp_fit[0] < 1: # Threshold to ensure it's not tilting the wrong way
            left_fit = temp_fit
    ...
    if left_fit is not None and right_fit is not None:
        ...
        lane_center_lookahead = l_top + (LANE_WIDTH_PX // 4)
        ...
        lane_center_lookahead = r_top - (LANE_WIDTH_PX // 4)
    ...
    car_camera_offset = -30

    cte = HALF_WIDTH - lane_center_bottom + car_camera_offset
    print("cte: ",cte)
    #set steering amount
    max_steer_threshold = HALF_WIDTH /2
    max_steer_amount = 92
    if(math.fabs(cte) > 0):
        steering = math.copysign((cte / max_steer_threshold * max_steer_amount), cte)
